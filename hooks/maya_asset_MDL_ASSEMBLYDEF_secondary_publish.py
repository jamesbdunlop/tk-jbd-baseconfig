...
class PublishHook(Hook):
    ...
    def _publish_gpu_for_item(self, item, output, work_template, primary_publish_path, sg_task, comment, thumbnail_path, progress_cb):
        ...
        # update fields with the group name:
        group_name              = item["name"].strip("|")
        log(app = None, method = '_publish_gpu_for_item', message = 'group_name: %s' % group_name, printToLog = False, verbose = configCONST.DEBUGGING)
        fields["grp_name"]      = group_name
        ...
        log(app = None, method = '_publish_gpu_for_item', message = 'gpuFileName: %s' % gpuFileName, printToLog = False, verbose = configCONST.DEBUGGING)
        log(app = None, method = '_publish_gpu_for_item', message = 'fileDir: %s' % fileDir, printToLog = False, verbose = configCONST.DEBUGGING)
        ...
        try:
            ...
            if cmds.objExists('dgSHD'):
                log(app = None, method = '_publish_gpu_for_item', message = 'Cleaning fileIn for gpu color', printToLog = False, verbose = configCONST.DEBUGGING)
                filesDict = adef_lib.fixDGForGPU()
                log(app = None, method = '_publish_gpu_for_item', message = 'filesDict: %s' % filesDict, printToLog = False, verbose = configCONST.DEBUGGING)
            ...
            for geo in cmds.listRelatives(group_name, children = True):
                if '%s_%s' % (configCONST.GEO_SUFFIX, configCONST.GROUP_SUFFIX) in geo:
                    ...
                    log(app = None, method = '_publish_gpu_for_item', message = 'geoGroup: %s' % geoGroup, printToLog = False, verbose = configCONST.DEBUGGING)
            ...
            log(app = None, method = '_publish_gpu_for_item', message = "gpuCache -startTime 1 -endTime 1 -optimize -optimizationThreshold 40000 -directory \"%s\" -fileName %s %s;" % (fileDir, gpuFileName, geoGroup), printToLog = False, verbose = configCONST.DEBUGGING)
            ...
            log(app = None, method = '_publish_gpu_for_item', message = 'Successfully exported gpu cache...', printToLog = False, verbose = configCONST.DEBUGGING)
    ...
    def _doRepresentationPublish(self, type, item, output, work_template, primary_publish_path, sg_task, comment, thumbnail_path, progress_cb):
        ...
        # update fields with the group name:
        group_name          = '%s%s' % (''.join(item["name"].strip("|").split('_%s' % configCONST.GROUP_SUFFIX)[0].split('_')), type)
        fields["grp_name"]  = group_name
        ## create the publish path by applying the fields with the publish template:
        publish_path        = publish_template.apply_fields(fields)
    ...
    def _publish_assemblyDefinition_for_item(self, mytasks, item, output, work_template, primary_publish_path, sg_task, comment, thumbnail_path, progress_cb):
        ...
        self._register_publish(publish_path, 
                               '%s_%s' % (''.join(group_name.split('_%s' % configCONST.GROUP_SUFFIX)[0].split('_')), configCONST.ASSEMBLYDEF_SUFFIX),
                               sg_task,
                               publish_version, 
                               tank_type,
                               comment,
