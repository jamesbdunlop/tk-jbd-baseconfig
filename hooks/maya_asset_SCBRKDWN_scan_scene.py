...
class ScanScene(Hook):
    ...
    def execute(self, **kwargs):
        ...
        log(app=None, method='ScanScene.execute', message='ref_nodes: {}'.format(ref_nodes, outputToLogFile=False, verbose=configCONST.DEBUGGING))
        for eachRef in ref_nodes:
            # get the path:
            ref_path = cmds.getAttr('{}.definition'.format(eachRef))
            log(app=None, method='ScanScene.execute', message='ref_path: {}'.format(ref_path, outputToLogFile=False, verbose=configCONST.DEBUGGING))
            ...
            if not '{}'.format(configCONST.RIG_SHORTNAME in ref_path):
                ## check the rig folder exists
                rigDir = os.path.dirname(ref_path.replace('{}'.format(configCONST.MODEL_SHORTNAME, '{}'.format(configCONST.RIG_SHORTNAME))))
                if os.path.isdir(rigDir):
                    ...
                    if getFiles:
                        ...
                        log(app=None, method='ScanScene.execute', message='reflastmodified: {}'.format(reflastmodified, outputToLogFile=False, verbose=configCONST.DEBUGGING))

                        ## Now if the rig is more recent than the model file...
                        if lastRigModified > reflastmodified:
                            cmds.warning('{} HAS A MORE RECENT RIG PUBLISH! PLEASE UPDATE TO THE RIG PATH IMMEDIATELY!'.format(eachRef))

                            ## Now make an out of date group
                            outDatedGroup = 'UPDATE_TO_RIG_ADEF_NOW_{}'.format(configCONST.GROUP_SUFFIX)
                            ...
                            try:
                                cmds.rename(eachRef, '{}_UPDATEMENOW'.format(eachRef))
                            except RuntimeError:
                                pass
        ...
        for file_node in cmds.ls(l=True, type="file"):
            ...
            # (maya uses C:/style/paths)
            path = cmds.getAttr("{}.fileTextureName".format(file_node).replace("/", os.path.sep))
